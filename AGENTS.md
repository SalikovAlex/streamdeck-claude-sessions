...
scripts/build-icons.sh                SVG in-app icons; qlmanage + sips for colour PNGs
...
- **Rendering assets:** `qlmanage` ignores SVG `text-anchor` — `scripts/build-marketplace-assets.mjs`
  ...
  exit after the shot). `scripts/build-icons.sh` writes white-on-transparent in-app SVGs directly and uses `qlmanage`+`sips` only for colour PNGs.
